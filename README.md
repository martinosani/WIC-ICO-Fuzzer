...
4. [COM Apartment Model](#4-com-apartment-model)
5. [Bug-Hunting Policy System](#5-bug-hunting-policy-system)
6. [Execution Profiles](#6-execution-profiles)
7. [Coverage Paths](#7-coverage-paths)
8. [Build System Requirements](#8-build-system-requirements)
9. [Building the Harness](#9-building-the-harness)
10. [Environment Setup](#10-environment-setup)
11. [Seed Corpus Preparation](#11-seed-corpus-preparation)
12. [WinAFL and TinyInst Setup](#12-winafl-and-tinyinst-setup)
13. [Launching Fuzzing Campaigns](#13-launching-fuzzing-campaigns)
14. [Monitoring and Triage](#14-monitoring-and-triage)
15. [Reproducing and Debugging Crashes](#15-reproducing-and-debugging-crashes)
16. [Configuration Reference](#16-configuration-reference)
17. [Source File Map](#17-source-file-map)
18. [Known Vulnerability Classes](#18-known-vulnerability-classes)
...
## 4. COM Apartment Model

The harness executes the Windows Imaging Component (WIC) decoder inside a
**Single-Threaded Apartment (STA)** by initializing COM with:
...
Most Windows desktop software that uses WIC — including the Windows shell,
image viewers, and many UI frameworks — runs image decoding inside STA
contexts. Executing the harness in STA therefore provides behavior that is
closer to real-world application environments, increasing the likelihood that
fuzzing reaches the same execution paths encountered in production systems.

### Why STA is used

Running the harness in a Single-Threaded Apartment provides several practical
advantages for fuzzing:

- **Improved stability in persistent-mode fuzzing**, since COM objects remain
  bound to a single thread.
- **Closer parity with common desktop application behavior**, which frequently
  uses STA execution models.
- **Reduced synchronization complexity**, because many WIC components assume
  STA-style access patterns.

### Experimental considerations

Fuzzing results generated by this harness **should not be directly compared
with results from a future Multi-Threaded Apartment (MTA) build**.

Changing the COM apartment model can influence internal decoder behavior,
including:

- codec initialization paths
- COM marshaling behavior
- interface invocation patterns
- thread-safety code paths within Windows imaging components

For this reason, **STA and MTA fuzzing campaigns should be treated as separate
experiments**, each with its own corpus, coverage results, and crash triage.

### Possible future extension

A future research variant of this harness may include an **MTA execution mode**
to explore additional concurrency-related code paths inside
`windowscodecs.dll`. Such campaigns should be run independently from the STA
configuration described here.

## 5. Bug-Hunting Policy System
...
cl /nologo /W3 /O2 /D HARNESS_MODE_FUZZ ^
   ...
   /link ole32.lib oleaut32.lib windowscodecs.lib shlwapi.lib version.lib ^
   /Fe:harness_c1.exe
```
...
## 13. Launching Fuzzing Campaigns
...
## 15. Reproducing and Debugging Crashes
...
## 18. Known Vulnerability Classes
