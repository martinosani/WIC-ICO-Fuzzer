...
BOOL config_load_ini(HARNESS_CONFIG* cfg)
{
    ...
    /* ---- Policy limits ---- */
    uval = (UINT)GetPrivateProfileIntW(INI_SECTION_HARNESS, INI_KEY_MAX_WIDTH,
        (INT)cfg->policy.maxWidth, iniPath);
    if (uval > 0 && uval <= 65536U) cfg->policy.maxWidth = uval;

    uval = (UINT)GetPrivateProfileIntW(INI_SECTION_HARNESS, INI_KEY_MAX_HEIGHT,
        (INT)cfg->policy.maxHeight, iniPath);
    if (uval > 0 && uval <= 65536U) cfg->policy.maxHeight = uval;

    uval = (UINT)GetPrivateProfileIntW(INI_SECTION_HARNESS, INI_KEY_MAX_FRAMES,
        (INT)cfg->policy.maxFrames, iniPath);
    if (uval > 0 && uval <= 65535U) cfg->policy.maxFrames = uval;

    uval = (UINT)GetPrivateProfileIntW(INI_SECTION_HARNESS, INI_KEY_MAX_BUFFER_MB,
        (INT)(cfg->policy.maxBufferBytes / (1024 * 1024)), iniPath);
    if (uval > 0 && uval <= 1024U)
        cfg->policy.maxBufferBytes = uval * 1024U * 1024U;

    /* ---- Iterations -- validated range [1, 1000000] ----
     * GetPrivateProfileIntW returns INT (signed).  A negative INI value
     * would silently wrap to ~4 billion iterations as UINT, so we validate
     * the raw signed result before assignment.
     */
    {
        INT rawIter = GetPrivateProfileIntW(INI_SECTION_HARNESS, INI_KEY_ITERATIONS,
            (INT)cfg->iterations, iniPath);
        if (rawIter > 0 && (UINT)rawIter <= 1000000U)
            cfg->iterations = (UINT)rawIter;
        /* else: keep compiled-in default -- invalid value is silently ignored */
    }
